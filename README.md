...
- 投喂计划：草稿修订自动升版，支持提交、批准、撤销和结束；批准时会校验水质与溶解氧阈值，同一养殖池同时只允许一个生效中的计划。
- 投喂建议：结合已批准或执行中的计划、24 小时内水质、天气和生长阶段，输出正常投喂、减量或暂停。
- 执行反馈：仅允许已批准或执行中的计划进入执行，记录实际量、现场溶解氧与反馈；计划在整个周期内可持续安排投喂，投喂完成后保持执行中，由主管在计划页结束后才变为已执行。
...
3. 在“投喂计划”创建草稿并提交，主管检查最新水质后批准；同一养殖池已有生效中的计划时，新计划会被拒绝，需先结束或撤销原计划。
4. 已批准或执行中的计划可输入天气窗口生成实时投喂建议。
5. 在“执行反馈”安排、开始并提交实际结果；首次投喂后计划进入 `executing`（执行中），周期内每天都可继续安排投喂。主管在计划页点“结束”后计划才变为 `executed`。
...
| `PATCH` | `/api/plans/:id/approve` | 主管批准并校验水质；同池已有生效中计划时拒绝 |
| `PATCH` | `/api/plans/:id/revoke` | 撤销待审或已批准计划 |
| `PATCH` | `/api/plans/:id/finish` | 主管结束计划，转为已执行 |
...
- 同一养殖池同一时间只能有一个已批准或执行中的计划；批准第二个时会被拒绝，需先结束或撤销原计划。
- 计划在周期内可重复安排投喂，投喂反馈提交后计划保持执行中；只有主管在计划页结束，计划才变为已执行，且结束前不得遗留待执行或执行中的安排。
