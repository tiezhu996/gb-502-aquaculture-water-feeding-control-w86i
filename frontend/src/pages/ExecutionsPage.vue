...
<template>
  <div class="page-stack">
    ...
    <el-dialog v-model="editorOpen" title="安排投喂执行" width="620px">
      <el-alert title="仅可选择已批准或执行中的计划；保存时将检查 24 小时内水质" type="info" :closable="false" show-icon />
      <el-form label-position="top" class="form-grid form-with-alert">
        ...
        <el-form-item label="生效计划"><el-select v-model="form.feedingPlanId" @change="onPlanChange"><el-option v-for="plan in availablePlans" :key="plan.id" :label="`${plan.name} · v${plan.version}`" :value="plan.id" /></el-select></el-form-item>
