...
<template>
  <div class="page-stack">
    <section class="metrics-grid">
      ...
      <MetricCard label="生效中" :value="activeCount" :icon="CircleCheck" tone="blue" hint="已批准或执行中" />
      <MetricCard label="生效计划日投喂" :value="`${formatNumber(dailyTotal)} kg`" :icon="TrendCharts" tone="green" />
    </section>
    ...
    <el-dialog v-model="transitionOpen" :title="transitionTitle" width="500px">
      <el-alert v-if="transitionAction === 'approve'" title="批准时将校验养殖池状态、最新水质和溶解氧阈值；同一养殖池只允许一个生效中的计划" type="warning" :closable="false" show-icon />
      <el-alert v-if="transitionAction === 'finish'" title="结束后计划归档为已执行，周期内不再接受新的投喂安排" type="warning" :closable="false" show-icon />
